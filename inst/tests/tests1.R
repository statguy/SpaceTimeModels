...
model$setData(coordinates[,c("UTMX","UTMY")], Piemonte_data$time, Piemonte_data$logPM10)
model$constructMesh(offset=c(10, 140), maxEdge=c(50, 1000), minAngle=c(26, 21), cutoff=0)
model$plotMesh()
model$setSpatialPrior()
model$setSmoothingModel()
model$getLinearModel()
model$buildObservationStack()
