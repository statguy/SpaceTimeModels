...
model$constructMesh(coords=cbind(coordinates$UTMX, coordinates$UTMY), locDomain=borders, offset=c(10, 140), maxEdge=c(50, 1000), minAngle=c(26, 21), cutoff=0)
model$plotMesh()
model$setSpatialPrior()
model$setSmoothingModel()
model$getLinearModel()
model$addObservationStack(coords=coordinates_validation[Piemonte_data$Station.ID, c("UTMX","UTMY")], time=Piemonte_data$time, response=Piemonte_data$logPM10, tag="obs")
model$addObservationStack(coords=coordinates_validation[Piemonte_data_validation$Station.ID, c("UTMX","UTMY")], time=Piemonte_data_validation$time, tag="val")
